...
export default function RaceResults({ raceId }: RaceResultsProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Official Results */}
      <div>
        <h3 className="text-lg font-semibold text-text mb-3">Official Results</h3>
        <div className="grid grid-cols-5 gap-2">
          {top5.map((entry) => (
            ...
            >
              <div className="text-sm text-text-muted mb-1">P{entry.position}</div>
              <div className="text-lg font-bold text-accent">{entry.code}</div>
              <div className="text-xs text-text-muted truncate">{entry.displayName.split(' ').pop()}</div>
            </div>
          ))}
        </div>
      </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-2xl font-bold text-accent">{myScore.points} pts</span>
            </div>
          </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border text-sm">
                    ...
                    <th className="px-4 py-2 text-center text-text-muted">Predicted</th>
                    <th className="px-4 py-2 text-center text-text-muted">Actual</th>
                    <th className="px-4 py-2 text-right text-text-muted">Points</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      ...
                      >
                        <td className="px-4 py-3">
                          ...
                          <span className="ml-2 text-text-muted text-sm">{item.code}</span>
                        </td>
                        <td className="px-4 py-3 text-center">
                          <span className="text-text-muted">P{item.predictedPosition}</span>
                        </td>
                        <td className="px-4 py-3 text-center">
                          {item.actualPosition ? (
                            <span className={
      ...
      {!myScore && (
        <div className="bg-surface-muted border border-border rounded-lg p-4 text-center">
          <p className="text-text-muted">You didn't submit a prediction for this race</p>
        </div>
      )}
    </div>
  );
